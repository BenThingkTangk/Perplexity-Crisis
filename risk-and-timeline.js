...
(function initRiskAndTimeline() {
  ...
  const CLAIM_DATA = {
    ...
    c11: { title: 'Cloudflare is the public front-door, CDN, WAF & bot layer', body: 'Live DNS and HTTP-header evidence (May 29, 2026) confirms Cloudflare for all of perplexity.ai, www.perplexity.ai, and api.perplexity.ai. NS records: jessica.ns.cloudflare.com, emerson.ns.cloudflare.com. A records: 104.18.26.48 / 104.18.27.48 (AS13335 Cloudflare, anycast). Headers: server: cloudflare, cf-ray, cf-cache-status: HIT (with non-zero age on static + API 404), cf-mitigated: challenge, __cf_bm cookie. This is the observable public path — what runs behind that edge stays Ask Perplexity.', src: 'dig NS · dig A · curl -I · ipinfo.io · May 29, 2026', row: 'confirmed', impact: 'high' },
    ...
    u3: { title: 'Internal routing behind the Cloudflare edge', body: 'The public front-door is Cloudflare (CONFIRMED via DNS + headers). What sits behind that edge — Cloudflare Workers usage, Perplexity-internal use of Cloudflare AI Gateway, semantic cache, and exact origin/inference topology — is obscured by the proxy by design. State Cloudflare as the public edge; treat anything inside as Ask Perplexity.', src: 'Cloudflare proxy hides origin by design', row: 'unknown', impact: 'high' },
